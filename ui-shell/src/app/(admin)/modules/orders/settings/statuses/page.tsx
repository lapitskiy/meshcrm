...
export default function OrdersSettingsStatusesPage() {
  ...
  const onSaveEdit = async () => {
    ...
    setError(null);
    try {
      const resp = await fetch(`${base}/orders/settings/statuses/${editingId}`, {
        method: "PUT",
        headers: { "content-type": "application/json", ...authHeaders() },
  ...
  const onDelete = async (id: string) => {
    ...
    setError(null);
    try {
      const resp = await fetch(`${base}/orders/settings/statuses/${id}`, {
        method: "DELETE",
        headers: authHeaders(),
      });
      if (!resp.ok) {
  ...
  const persistOrder = async (ordered: StatusItem[]) => {
    const resp = await fetch(`${base}/orders/settings/statuses/reorder`, {
      method: "POST",
      headers: { "content-type": "application/json", ...authHeaders() },
    ...
    <div className="space-y-6">
      <div className="rounded-2xl border border-gray-200 bg-white px-5 py-6 dark:border-gray-800 dark:bg-white/[0.03]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <Input value={name} onChange={(e: any) => setName(e.target.value)} placeholder="Например: В работе" />
          </div>
          <div>
            <Label>Цвет</Label>
            <input
      ...
      <div className="rounded-2xl border border-gray-200 bg-white px-5 py-6 dark:border-gray-800 dark:bg-white/[0.03]">
        ...
        <div className="text-xs text-gray-500 dark:text-gray-400 mb-3">
          Перетащите строку мышью, чтобы изменить порядок вывода.
        </div>

...
export default function OrdersSettingsWorkTypesPage() {
  ...
  const loadCategories = async () => {
    ...
    setCategories(data);
    if (!newCategoryId && data.length) {
      setNewCategoryId(data[0].id);
    }
  };

  const loadWorkTypes = async (categoryId: string) => {
    const qs = categoryId ? `?service_category_id=${encodeURIComponent(categoryId)}` : "";
    const resp = await fetch(`${base}/orders/settings/work-types${qs}`, {
      cache: "no-store",
      headers: authHeaders(),
    });
    if (!resp.ok) {
    ...
    setItems((await resp.json()) as WorkType[]);
  };
  ...
  useEffect(() => {
    (async () => {
      try {
        await loadWorkTypes(filterCategoryId);
      } catch (e: any) {
  ...
  const onCreate = async () => {
    ...
    setError(null);
    try {
      const resp = await fetch(`${base}/orders/settings/work-types`, {
        method: "POST",
      ...
      setError(e?.message || "failed to create work type");
    } finally {
      setBusy(false);
    }
  };
  ...
  const onSaveEdit = async () => {
    ...
    setError(null);
    try {
      const resp = await fetch(`${base}/orders/settings/work-types/${editingId}`, {
        method: "PUT",
        headers: { "content-type": "application/json", ...authHeaders() },
      ...
      setError(e?.message || "failed to update work type");
    } finally {
      setActionBusy(false);
    }
  };
  ...
  const onDelete = async (id: string) => {
    ...
    setError(null);
    try {
      const resp = await fetch(`${base}/orders/settings/work-types/${id}`, {
        method: "DELETE",
        headers: authHeaders(),
      });
      if (!resp.ok) {
      ...
      setError(e?.message || "failed to delete work type");
    } finally {
      setActionBusy(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="rounded-2xl border border-gray-200 bg-white px-5 py-6 dark:border-gray-800 dark:bg-white/[0.03]">
        <h3 className="mb-4 font-semibold text-gray-800 text-theme-xl dark:text-white/90">Новый вид работ</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <Input value={newName} onChange={(e: any) => setNewName(e.target.value)} placeholder="Например: Замена масла" />
          </div>
        </div>
        <div className="mt-4">
      ...
      <div className="rounded-2xl border border-gray-200 bg-white px-5 py-6 dark:border-gray-800 dark:bg-white/[0.03]">
        <div className="flex items-center justify-between gap-4 mb-4">
          <h3 className="font-semibold text-gray-800 text-theme-xl dark:text-white/90">Список работ</h3>

...
export default function WbFinancesPage() {
  ...
  const load = async (mAgo: number) => {
    ...
    try {
      const r = await fetch(
        `${getGatewayBaseUrl()}/marketplaces/wb/finances?months_ago=${encodeURIComponent(String(mAgo))}&mode=${encodeURIComponent(mode)}`,
        {
  ...
  return (
    <div className="p-6 max-w-3xl">
      <h1 className="text-2xl font-semibold mb-2">WB → Финансы</h1>
      ...
      <div className="flex items-center gap-3 mb-4">
        ...
        >
          {loading ? "Загружаю..." : mode === "live" ? "Обновить из WB" : "Загрузить из БД"}
        </button>
        <select
        ...
        >
          ...
          <option value="live">WB - сохранить в БД</option>
        </select>
        <select
          className="h-11 rounded-lg border border-gray-300 px-3 text-sm dark:border-gray-700 dark:bg-gray-900"
          value={monthsAgo}
          ...
          {Array.from({ length: 25 }).map((_, i) => {
            ...
            return (
              <option key={v} value={v}>
                {v === 0 ? "Текущий месяц" : v === 1 ? "Прошлый месяц" : `${v} месяца назад`}
              </option>
            );
          })}
        </select>
                ...
                {Array.isArray(entries) &&
                  entries.map((e: any, idx: number) => (
                    <div key={idx} className="rounded-lg bg-gray-50 p-3 text-sm dark:bg-white/[0.03]">
                      ...
                      <div className="text-gray-700 dark:text-gray-300">
                        Цена продажи: ₽{e.sale_price}, шт: {e.quantity}
                      </div>
                      <div className="text-gray-700 dark:text-gray-300">
